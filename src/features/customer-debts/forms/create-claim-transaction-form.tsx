...
import { accountTransactionFormSchema } from "@/features/account-transactions/schemas/account-transaction-schema";
import type { CreateClaimModel } from "@/features/account-transactions/models/create-claim-model";
import type { ICreateTransactionProps } from "@/features/account-transactions/props/create-accounttransaction-props";
...
export function CreateClaimForm({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    const model = form.getValues() as CreateClaimModel;
    if(model.claim > balance){
      toast.error("Alacak miktarı mevcut bakiyeden fazla olamaz.");
      return;
    }
    onSubmit(model);
    await onCreatedClaim?.();
  };

  return (
    <div className={cn("flex flex-col gap-6", className)} {...props}>
      <h4 className="font-bold">
        {customerName} {customerSurname} İçin Tahsilat Oluştur
      </h4>
      <Form {...form}>
